...
- 当前只支持手动输入 `otpauth://` 参数或 Base32 密钥；二维码扫描暂不实现。
...
- `appSettings` 是可读、可手工编辑的软件设置，保存主题模式、配色/语言标识、自动锁定时长和剪贴板清除时长；不做完整性校验。
- 导出不显示密码输入框，直接复用当前主密码派生的密钥；导出前提示用户记住主密码。
- 导入时必须输入当前主密码（4-20 个 Unicode 字符，可含中文、英文、数字和符号），通过 AES-GCM 认证后才允许恢复。
- 导入成功前只在内存中解析，用户确认后整体替换当前密码库与软件设置；失败不修改现有数据。
- 使用 Android Storage Access Framework 的 `CreateDocument`/`OpenDocument` 申请单文件读写授权，不申请广泛存储权限。
- 剪贴板由系统 `ClipboardManager` 写入；Android 应用写入前台剪贴板不需要额外运行时权限。
...
- 数据流：主密码 → KEK/本地 DEK → 内存密码库；导出时读取当前 KEK → 生成 `.acc` → `CreateDocument` 写入；导入时 `OpenDocument` 读取 → 主密码验证与解密 → 用户确认 → 原子保存。
...
- 导出无需输入密码并提示记住主密码；导入要求 4-20 字符主密码，错误密码或认证失败时拒绝恢复且不改动当前数据。
