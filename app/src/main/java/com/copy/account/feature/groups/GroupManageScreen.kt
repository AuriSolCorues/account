...
internal fun GroupManageScreen(
    ...
    actionGroup?.let { group ->
        AppBottomSheet(onDismiss = { actionGroup = null }, skipPartiallyExpanded = true, verticalArrangement = Arrangement.spacedBy(10.dp)) {
            Text(group.name, style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.Bold)
            ActionSheetRow("修改分组名称") {
                actionGroup = null
                dialogGroup = group
                dialogText = group.name
            }
            if (group.kind == GroupKind.CUSTOM) {
                ActionSheetRow("删除此分组", color = MaterialTheme.colorScheme.error) {
                    actionGroup = null
                    deleteConfirmGroup = group
                }
            }
            ActionSheetRow("取消", muted = true) { actionGroup = null }
...
internal fun LazyItemScope.GroupManageItem(
    ...
    Card(
        ...
        Row(modifier = Modifier.fillMaxWidth().padding(horizontal = 12.dp, vertical = 8.dp), verticalAlignment = Alignment.CenterVertically) {
            ...
            if (!fixed) DangerButton("删除", onClick = delete)
        }
    }
}
